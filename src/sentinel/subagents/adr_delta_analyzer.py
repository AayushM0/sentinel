...
class ADRDeltaAnalyzer:
    ...
    async def run(
        ...
        if not request.git_diff.files:
            ...
            self._persist_if_store_provided(request, report, session_store)
            return report
    ...
    def _extract_query_context(self, request: DeltaRequest) -> str:
        """Derive search query keywords from touched files and diff hunks."""
        terms: list[str] = []
        for path in request.touched_files:
        ...
        for file_diff in git_diff.files:
            ...
            for line in added_lines:
                ...
                for adr in enforceable_adrs:
                    if adr.code_pattern and adr.code_pattern in clean_line:
                        violations.append(
                            f"{file_diff.path}: Code matches prohibited pattern in {adr.id} ('{adr.title}')"
                        )
        return violations

    def _persist_if_store_provided(
        self,
        request: DeltaRequest,
        report: DeltaReport,
...
if __name__ == "__main__":
    ...
    async def _self_test() -> None:
        ...
        print("adr_delta_analyzer.py standalone self-check passed successfully.")
